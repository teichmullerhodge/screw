...
pub fn get_sketch_config_dir_path() -> std::path::PathBuf {
    dirs_next::config_dir()
        .expect("Could not open the config dir")
        .join("sketch")
}

pub fn get_sketch_projects_path() -> std::path::PathBuf {
    dirs_next::document_dir()
        .expect("Could not find the document dir")
        .join("sketch")
        .join("projects")
}
...
pub fn collect_user_projects() -> Result<Vec<UserTemplateResult>, UserProjectsOperation> {
    ...
    let configs: ConfigTemplatesLocation = serde_json::from_str(
        &contents.unwrap_or("".to_string()),
    )
    .unwrap_or(ConfigTemplatesLocation {
        templates: Vec::new(),
    });
    for paths in configs.templates {
        ...
        let project_manifest: Result<UserTemplate, serde_json::Error> =
            serde_json::from_str(&sketch_string);
...
pub fn save_template_to_config_file(template_path: std::path::PathBuf) -> ConfigProjectsResult {
    ...
    if !projects_file_exists {
        let create_res = create_file(
            config_file_path.clone(),
            Some(SKETCH_CONFIG_FILE_INITIAL_VALUE.to_string()),
        );
    ...
    let mut config_file: ConfigTemplatesLocation = serde_json::from_str(
        &contents.unwrap_or("".to_string()),
    )
    .unwrap_or(ConfigTemplatesLocation {
        templates: Vec::new(),
    ...
    config_file
        .templates
        .push(template_path.display().to_string()); // attention to this fuckness.
...
pub fn execute_manifest(app: tauri::AppHandle, template: TemplateManifest) -> (Option<String>, ManifestOperation) {
    ...
    let root = get_sketch_projects_path()
        .join(template.language.clone())
        .join(template.category.clone())
        .join(template.name.clone());
    ...
    match config_res {
        ConfigProjectsResult::ConfigOk => {
            eprintln!("Properly appended the project to the config file.")
        }
